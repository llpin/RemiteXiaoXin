...
//	//是否登录过
//	if($type == 'there'){
//		$res = $Db->getOne("select * from quanrun_info where openid='$openid'");	//查询数据是否存在
//		if (!$res) {
//			showsuc("还未登录过",array('status'=>0,'openid'=>$openid));
//		}
//		else{
//			$rank = $Db->getOne("select * from quanrun_info where openid='$openid'");
//			$code = $Db->getOne("select * from quanrun_code where code='{$rank['code']}' ");
//			if($code){
//				showsuc("邀请码正确",array('status'=>1,'code'=>0,'openid'=>$openid));
//			}else{
//				showsuc("邀请码错误",array('status'=>1,'code'=>1,'openid'=>$openid));
//			}
//		}
//	}
	//登录
	if($type == 'login'){
		$nickname = $user_info['nickname'];			//微信昵称
		$headimgurl = $user_info['headimgurl'];		//头像链接
		$res = $Db->getOne("select * from quanrun_info where openid='$openid'");	//查询数据是否存在
		if (!$res) {
			$Db->add("quanrun_info", array('nickname' => $nickname, 'headimgurl' => $headimgurl, 'openid' => $openid,));
			showsuc("登录成功");
		}else{
			showsuc("已经登录过");
		}
	...
	//个人中心
	if($type == 'data'){
		$res = $Db->getOne("select * from quanrun_info where openid='$openid'");
	...
	if($type == 'perfect'){
		...
		$res = $Db->getOne("select * from quanrun_info where phone='$phone' and openid !='$openid'");
		if($res){
			showerr("该手机号码已被注册");
		}else{
			if(empty($password)){
				$Db->update("quanrun_info", array('name'=>$name,'firm'=>$firm,'post'=>$post,'phone'=>$phone,'code'=>$code,'email'=>$email),"openid='$openid'");
				showsuc("修改成功");
			}else{
				$Db->update("quanrun_info", array('name'=>$name,'firm'=>$firm,'post'=>$post,'phone'=>$phone,'code'=>$code,'email'=>$email,'password'=>$password),"openid='$openid'");
				showsuc("提交成功");
			}
		}
	}
...
//		$info = $Db->getAll("select * from quanrun_course where type='$type'");			//课程内容
...
//		}
	...
	if($type == 'code'){
		$code = $_REQUEST['code'];
		$Db->update("quanrun_info",array('code'=>$code),"openid='$openid'");
		$rank = $Db->getOne("select * from quanrun_info where openid='$openid'");
